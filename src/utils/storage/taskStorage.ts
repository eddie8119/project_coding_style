...
export const TASK_STORAGE_PREFIX = 'task_';
 ...
 * 初始化任務本地數據
 * @param projectId 專案ID
 * @param fetchedTask API 獲取的任務數據
 * @returns 本地任務數據的 ref
 */
export function initTaskLocalStorage<T>(projectId: string, fetchedTask: T): Ref<T> {
  const localTask = ref<T>(fetchedTask as T) as Ref<T>;
  const storageKey = getTaskStorageKey(projectId);
  ...
  if (storedData) {
    ...
    if (storedDate < fetchedDate) {
      // 資料庫數據更新，使用資料庫數據
      localTask.value = fetchedTask as T;
      saveToLocalStorage(storageKey, localTask.value);
    } else {
      // 本地數據更新，使用本地數據
      localTask.value = storedData;
    }
  } else {
    // 沒有本地數據，使用資料庫數據
    localTask.value = fetchedTask as T;
 ...
 * @returns 是否有更改
 */
export function saveTaskToLocalStorage<T>(projectId: string, taskData: T): boolean {
  const storageKey = getTaskStorageKey(projectId);
  const storedData = getFromLocalStorage<T>(storageKey);

  // 檢查數據是否有變化
  if (JSON.stringify(storedData) !== JSON.stringify(taskData)) {
    saveToLocalStorage(storageKey, taskData);
    return true; // 有更改
  }
  ...
  const handler = (event: BeforeUnloadEvent) => {
    if (hasChanges.value) {
      ...
      try {
        ...
        const url = `/api/tasks/${projectId}`;
        const data = new Blob([JSON.stringify(taskData.value)], { type: 'application/json' });
